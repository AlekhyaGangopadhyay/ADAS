...
# ── Braking constants ────────────────────────────────────
BRAKE_DECEL_SOFT  = 0.12
BRAKE_DECEL_HARD  = 0.30
BRAKE_DECEL_EMERG = 0.60
...
# === NOVELTY 1: RATIONAL B-SPLINE (NURBS-lite approach) ===
# By incorporating adjustable weights (w0 to w3) into the standard B-spline blend functions,
# we unlock Rational B-Splines. This enables precise tuning of curve tension, allowing 
# the trajectory to closely hug specific control points for higher accuracy and preventing overshoots.
def rational_b_spline(p0, p1, p2, p3, w0, w1, w2, w3, n=60):
    ...
    for i in range(n):
        ...
        # Apply weights to blending functions
        denominator = (b0 * w0) + (b1 * w1) + (b2 * w2) + (b3 * w3)
        if denominator == 0: denominator = 1e-6
        
        weight_p0 = (b0 * w0) / denominator
        weight_p1 = (b1 * w1) / denominator
        weight_p2 = (b2 * w2) / denominator
        weight_p3 = (b3 * w3) / denominator
        
        pt = weight_p0 * p0 + weight_p1 * p1 + weight_p2 * p2 + weight_p3 * p3
        pts.append(pt.tolist())
    return pts

def build_spline(wps, w_in=None, n=40):
    wp = [wps[0], wps[0]] + list(wps) + [wps[-1], wps[-1]]
    if w_in is None:
        w_in = [1.0] * len(wps)
    weights = [w_in[0], w_in[0]] + list(w_in) + [w_in[-1], w_in[-1]]
    
    path = []
    for i in range(len(wp) - 3):
        seg = rational_b_spline(
            wp[i], wp[i+1], wp[i+2], wp[i+3], 
            weights[i], weights[i+1], weights[i+2], weights[i+3], n
        )
        if i > 0: seg = seg[1:]
        path.extend(seg)
    return path
...
class Sim:
    def __init__(self):
        ...
        pygame.display.set_caption("Autonomous Driving  v10  |  B-Spline Novelty Edition")
    ...
    def _plan_change(self, to_lane):
        ...
        # === NOVELTY 2: Velocity-Adaptive Path Generation ===
        # The lane change 'run' distance is dynamically determined based on the ego's 
        # speed and the relative speed to the target car. This promotes comfortable,
        # longer lane changes at high speeds, and aggressive, shorter merges at lower speeds.
        if target:
            rel_speed = max(0, ego.speed - target.speed)
            run = min(max(180, ego.speed * 30 + rel_speed * 15), 360)
        else:
            run = min(max(150, ego.speed * 30), 320)
            
        # Create control points
        wps = [
            [x0, y0],
            [x0 + run * 0.15, y0 + dy * 0.05],
            [x0 + run * 0.45, y0 + dy * 0.5],
            [x0 + run * 0.80, y0 + dy * 0.95],
            [x0 + run, y1],
            [x0 + run + 80, y1]
        ]
        
        # === NOVELTY 1 CONTINUED: Weight Assignments ===
        # By increasing the weight of the intermediate control points (e.g. up to 2.5),
        # the Rational B-Spline avoids loose curves and snaps closer to the intended
        # safety bounds. Weights correspond to the waypoints in 'wps'.
        weights = [1.0, 1.0, 2.5, 2.5, 1.0, 1.0]
        
        path = build_spline(wps, w_in=weights)
